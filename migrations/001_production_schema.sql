...
-- ─── Production Orders ────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS mfgp_production_orders (
...
CREATE INDEX IF NOT EXISTS idx_production_orders_tenant_id
  ON mfgp_production_orders (tenant_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_production_orders_status
  ON mfgp_production_orders (tenant_id, status);

-- ─── Bill of Materials ────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS mfgp_bill_of_materials (
  id                   TEXT PRIMARY KEY NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_bom_tenant_order
  ON mfgp_bill_of_materials (tenant_id, production_order_id);

-- ─── Quality Checks ───────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS mfgp_quality_checks (
  id                   TEXT PRIMARY KEY NOT NULL,
  tenant_id            TEXT NOT NULL,
  production_order_id  TEXT NOT NULL REFERENCES mfgp_production_orders(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_quality_checks_tenant_order
  ON mfgp_quality_checks (tenant_id, production_order_id);

CREATE INDEX IF NOT EXISTS idx_quality_checks_result
  ON mfgp_quality_checks (tenant_id, result);
